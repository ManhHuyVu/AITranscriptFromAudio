 ...
 *
 * Assignment requirement: "Provide a high performance implementation
 * (able to handle > 200 concurrent blocking HTTP requests) within the
 * confines of a single Java process."
 *
 * Strategy:
 *   Uses {@code @SpringBootTest} with a real Tomcat server (random port)
 *       to test the full request pipeline — not just the controller layer.
 *   The {@link TranscriptionService} is mocked via {@code @MockitoBean}
 *       so we never call the real OpenAI API. The mock returns immediately,
 *       isolating the concurrency test to the server's request handling.
 *   A {@link CountDownLatch} synchronises all threads so they fire
 *       simultaneously, maximising thread contention on Tomcat's virtual
 *       thread pool.
 *   Three test tiers: 100, 200, and 400 concurrent requests.
 *
 * Virtual threads note: With {@code spring.threads.virtual.enabled=true},
 ...
 * threads handle it with minimal memory overhead.
 */
...
class ConcurrencyStressTest {
     ...
     *
     * Flow:
     *   Create a fixed thread pool of {@code concurrency} threads
     *   Set up a CountDownLatch(1) as a starting gun — all threads wait
     *       on it, then fire simultaneously when countDown() is called
     *   Each thread: waits for the latch, then POSTs to /api/v1/transcription
     *       using MockMvc (which runs the full Spring MVC pipeline in-process)
     *   Collect all Futures and verify every request returned HTTP 200
     *
    ...
    private void runConcurrencyTest(int concurrency) throws Exception {
        ...
        // This is faster and more reliable for concurrency tests 
        // than using TestRestTemplate or WebClient which go through real HTTP
     ...
     *
     * Baseline load test. A platform thread pool (default 200 threads)
     * handles this comfortably. Virtual threads handle it trivially
     */
    @Test
     ...
     *
     * Double the minimum requirement — tests headroom and proves
